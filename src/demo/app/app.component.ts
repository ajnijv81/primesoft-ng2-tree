...
import { Ng2TreeSettings, NodeEvent, RenamableNode, TreeModel, NodeMenuItem } from '../../../index';
...
@Component({
  selector: 'app',
  templateUrl: 'app.component.html',//Customised
  styles: [
    `
    .tree-info {
        flex: 1 0 100%;
...
export class AppComponent implements OnInit {
  ...
  //Customised
  isMoveClickActive: boolean;
  isMoveClicked: boolean;

  isCopyClickActive: boolean;
  isCopyClicked: boolean;

  displayMoveOrCopy: boolean;
  displayUndoAndPaste:boolean;
  ////
  ...
  public icons: TreeModel = {
    ...
    settings: {
      ...
      menuItems: [
        { action: NodeMenuItemAction.NewFolder, name: 'Add Folder', cssClass: 'fa fa-folder-o fa-lg', display: true },
        { action: NodeMenuItemAction.NewTag, name: 'Add File', cssClass: 'fa fa-file-o fa-lg', display: true },
        { action: NodeMenuItemAction.Rename, name: 'Rename', cssClass: 'node-menu-item-icon rename', display: true },
        { action: NodeMenuItemAction.Remove, name: 'Remove', cssClass: 'node-menu-item-icon remove', display: true },
        { action: NodeMenuItemAction.Custom, name: 'Move', cssClass: 'fa fa-hand-rock-o', display: true },
        { action: NodeMenuItemAction.Custom, name: 'Copy', cssClass: 'fa fa-file-zip-o', display: true },
        { action: NodeMenuItemAction.Custom, name: 'Undo Action', cssClass: 'fa fa-hand-rock-o', display: false },
        { action: NodeMenuItemAction.Custom, name: 'Paste', cssClass: 'fa fa-file-zip-o', display: false }
      ]
  ...
  public custom: TreeModel = {
    settings: {
      menuItems: [
        ...
        { action: NodeMenuItemAction.Move, name: 'Custom Action', cssClass: '' }
      ]
    },
    value: 'TestParent',
  ...
  public onMenuItemSelected(e: MenuItemSelectedEvent) {
    ...
    this.toggleMenu(e.selectedItem, true); //Customised
  }
  //Customised
  toggleMenu = (action: string, status: boolean) =>{
    this.isMoveClicked = (action === 'Move') ? true : false;
    this.isCopyClicked = (action === 'Copy') ? true : false;
    this.displayMoveOrCopy  =  (this.isMoveClicked || this.isCopyClicked) ? false : true;
    this.displayUndoAndPaste = !this.displayMoveOrCopy; //;

    this.icons.settings.menuItems.forEach(item => {   
      // enable undo and paste and disabled move copy if move or copy clicked
      if(!this.displayMoveOrCopy && (item.name === 'Undo Action' || item.name === 'Paste')){
          item.display = status;        
       } else if(item.name === action) { // for move or copy
        item.display = !status;    
      } 
      // disable undo and paste and enable move copy if undo and paste clicked 
       if(this.displayMoveOrCopy && !this.displayUndoAndPaste){
        if(item.name === 'Undo Action' || item.name === 'Paste'){
          item.display = !status;        
        } else if(item.name === 'Move' || item.name === 'Copy') {
          item.display = status;    
        }
       }

       //toggle Copy click right after move ans vise versa
